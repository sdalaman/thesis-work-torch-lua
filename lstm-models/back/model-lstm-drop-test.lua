...
model_load_path = path.."model-impl/lstm-models/model-files/10000/new/2/"
...
--for 10000 new 1
--batch_size = 50;init = 1 weight_decay = 1e-4
scenarios[#scenarios+1] = { lr = 0.01;prev_lr = 0.01;alpha = 0.95;lr_decay = 1e-4;weight_decay = 1e-4; lr_list = nil ;emb_size = 64;hidden_size = 128;momentum=0.5; threshold = 50 ;dump_frq = 50;max_epoch = 5000;epoch=400;batch_size = 100;init = 1;max_seq_len = max_sentence_len;win_size = 1;opm_name = 'rmsprop';opm = optim.rmsprop; train_score = 0;test_score = 0;num_of_samples = 0;testcorpus1 = "english.10000.tok";testcorpus2 = "turkish.10000.tok";test_size = 100;err_test_size = 1000;out_file = "test-lstm-10000.txt"; dsize ="1000";max_cv = 10;coefL1=0;coefL2=0}

else

--for 10000 morph new 1
scenarios[#scenarios+1] = { lr = 0.01;prev_lr = 0;alpha = 0.95;lr_decay = 1e-4;weight_decay = 1e-4; lr_list = nil ;emb_size = 64;hidden_size = 128;momentum=0.5; threshold = 50 ;dump_frq = 50;max_epoch = 5000;epoch=0;batch_size = 100;init = 1;max_seq_len = max_sentence_len;win_size = 1;opm_name = 'rmsprop';opm = optim.rmsprop; train_score = 0;test_score = 0;num_of_samples = 0;testcorpus1 = "english.10000.tok.morph";testcorpus2 = "turkish.10000.tok.morph";test_size = 100;err_test_size = 1000;out_file = "test-lstm-10000.txt"; dsize ="1000";max_cv = 10;coefL1=0;coefL2=0}
...
function model_load(model,prm_list)
  ...
  lstm = model.mdlSec:get(1):get(1):get(1):get(3)
  lstm.mono = true
  lstm = model.mdlPri:get(1):get(1):get(1):get(3)
  lstm.mono = true
...
function model_dropout_set(model)
  drop = model.mdlPri:findModules('nn.Dropout')
  for i = 1, #drop do
    drop[i].mono = true
    drop[i].flag = true
  end  
  drop = model.mdlSec:findModules('nn.Dropout')
  for i = 1, #drop do
    drop[i].mono = true
    drop[i].flag = true
  end  
...
for i=1,#scenarios do
  ...
  loss_logger = optim.Logger(fhead..'_loss_log_'..i..'-'..prm_list["lr"]..'-'..prm_list["emb_size"]..'-'..prm_list["hidden_size"]..'-'..prm_list["max_seq_len"],true,prepheadstr(prm_list))
  grad_logger = optim.Logger(fhead..'_grad_log_'..i..'-'..prm_list["lr"]..'-'..prm_list["emb_size"]..'-'..prm_list["hidden_size"]..'-'..prm_list["max_seq_len"],true,prepheadstr(prm_list))
  score_logger = optim.Logger(fhead..'_score_log_'..i..'-'..prm_list["lr"]..'-'..prm_list["emb_size"]..'-'..prm_list["hidden_size"]..'-'..prm_list["max_seq_len"],true,prepheadstr(prm_list))
  ...
  if model_load_flag == true then
    ...
    --tst_err = test_err(modelLSTM,testInp1,testInp2,prm_list["test_size"])
    print("Train Data Score : "..prm_list["train_score"].." - Test Data Score : "..prm_list["test_score"].."\n")
