...
function Corpus:__init(fname,d_path)
  self.vocab_idx = 1
  self.vocab_map = {['<pad>'] = 0,['_UNK_'] = 1}
  self.vocab_cnt = {['<pad>'] = {0,0,0},['_UNK_'] = {0,0,0}}
  self.vocab_rpl = {}
  self.longest_seq = 0
...
function Corpus:load_data(fname,max_len)
   ...
   for i = 1 , #self.sequences do
     ...
     for j = 1, words_len do 
       if self.vocab_rpl[words[j]] == nil then
         self.vocab_rpl[words[j]] = 0
       end
     end
   end
  ...
  local totseq=0
   for i = 1, #self.sequences do
     ...
     for j = 1, #words do 
       ...
       if self.vocab_cnt[words[j]] == nil then
         self.vocab_cnt[words[j]] = 1
       else
         self.vocab_cnt[words[j]] = self.vocab_cnt[words[j]] + 1         
       end
  ...
  for i = 1, #self.sequences do
     ...
     for j = 1, #words do 
       if self.vocab_map[words[j]] == nil then
         self.vocab_idx = self.vocab_idx + 1
         self.vocab_map[words[j]] = self.vocab_idx
         self.vocab_cnt[words[j]] = {1,i,j}
       else
         self.vocab_cnt[words[j]][1] = self.vocab_cnt[words[j]][1] + 1         
       end
       x[i][j] = self.vocab_map[words[j]] 
     end
  end
  local single_word_cnt = 0
  -- convert every words with freq = 1 to UNK
  for k,v in pairs(self.vocab_cnt) do
    if v[1] == 1 then
      single_word_cnt = single_word_cnt + 1
      x[v[2]][v[3]] = self.vocab_map['_UNK_']
    end
  end  
  print(string.format("Number of words = %d , mean sntc len %4.2f", self.vocab_idx,totseq/#self.sequences))
  print(string.format("Number of single words = %d ",single_word_cnt ))
  self.no_of_words = self.vocab_idx
  return x
end
...
function testcorpus_shapedataLSTM(corpus,test_corpus)
  ...
  for i = 1, #test_corpus.sequences do
    ...
    word_cnt = word_cnt + #words
    words = test_corpus:paddingLSTM(words)
    for j = 1, #words do 
      if words[j] == '_UNK_' then
        unk_cnt2 = unk_cnt2 + 1
      end
      if corpus.vocab_map[words[j]] == nil then
         --x[i][j] = 0  <pad>
         x[i][j] = corpus.vocab_map['_UNK_']  -- '_UNK_'
         unk_cnt = unk_cnt + 1
      else
  ...
  test_corpus.no_of_words = word_cnt
  print(string.format("Number of actual unknown words = %d ", unk_cnt2))
  print(string.format("Number of unknown words = %d / number of words %d %4.2f%%", unk_cnt,test_corpus.no_of_words,100*unk_cnt/test_corpus.no_of_words))
  return x
end
...
function Corpus:shape_dataBiLSTM()
  ...
  local totseq=0
   for i = 1, #self.sequences do
     ...
     for j = 1,#wordsFrw do
       if self.vocab_rpl[wordsFrw[j]] == 0 then
         self.vocab_rpl[wordsFrw[j]] = 1
         wordsFrw[j] = '_UNK_'
       end
     end
   ...
   for i = 1, #test_corpus.sequences do
     ...
     for j = 1, #wordsFrw do 
      if corpus.vocab_map[wordsFrw[j]] == nil then
         fwd[i][j] = corpus.vocab_map['_UNK_']  -- '_UNK_'
         unk_cnt = unk_cnt + 1
      else
     ...
     for j = 1, #wordsBck do 
       if corpus.vocab_map[wordsBck[j]] == nil then
         bck[i][j] = corpus.vocab_map['_UNK_']  -- '_UNK_'
      else
