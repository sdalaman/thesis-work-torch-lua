...
function Corpus:paddingLSTM(sequence)
  -- if extracting word embeddings! 
  new_sequence = {}
  mlen = math.min(self.longest_seq,#sequence)
  for i = 1 , mlen do
    new_sequence[i] = sequence[i]
  end
  for i = mlen+1, self.longest_seq do
    new_sequence[i] = '<pad>'
  end
  return new_sequence
end
...
function Corpus:shape_dataLSTM()
  ...
  local totseq=0
   for i = 1, #self.sequences do
     ...
     totseq=totseq+#words
     for j = 1,#words do
       if self.vocab_rpl[words[j]] == 0 then
         self.vocab_rpl[words[j]] = 1
         words[j] = '_UNK_'
       end
     end
...
function testcorpus_shapedataLSTM(vocabPri,test_corpus)
  local unk_cnt = 0
  local word_cnt = 0
  local unk_cnt2 = 0
  ...
  for i = 1, #test_corpus.sequences do
    ...
    for j = 1, #words do 
      ...
      if vocabPri[words[j]] == nil then
         x[i][j] = vocabPri['_UNK_']  -- '_UNK_'
         unk_cnt = unk_cnt + 1
      else
         x[i][j] = vocabPri[words[j]] 
      end
    end
  end
...
function testcorpus_shapedataBiLSTM(vocabPri,test_corpus)
  local unk_cnt = 0
  local word_cnt = 0
  ...
  local bck = torch.zeros(#test_corpus.sequences, test_corpus.longest_seq)
   for i = 1, #test_corpus.sequences do
     ...
     for j = 1, #wordsFrw do 
      if vocabPri[wordsFrw[j]] == nil then
         fwd[i][j] = vocabPri['_UNK_']  -- '_UNK_'
         unk_cnt = unk_cnt + 1
      else
         fwd[i][j] = vocabPri[wordsFrw[j]] 
      end
     end
     bck[i]=fwd[i]:index(1,torch.linspace(test_corpus.longest_seq,1,test_corpus.longest_seq):long()) 
   end
   test_corpus.no_of_words = word_cnt
  print(string.format("Number of unknown words = %d / number of words %d %4.2f%%", unk_cnt,test_corpus.no_of_words,100*unk_cnt/test_corpus.no_of_words))
   return fwd,bck
