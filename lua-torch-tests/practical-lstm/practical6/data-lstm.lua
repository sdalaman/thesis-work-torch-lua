...
function Corpus:load_data(fname,max_len,max_sent)
   ...
   if max_sent > #tmpsequences then
     max_sent = #tmpsequences / 100
     print("max sent too big, it is updated to "..max_sent.."\n")
   end
   self.sequences = {}
   rlist = false
   if #sellist == 0 then
...
function get_sentence_hist(sequences)
  local x = torch.zeros(#sequences)
  for i = 1 , #sequences do
    local words = stringx.split(sequences[i],'\t') -- tokenized with tab
    x[i] = #words
  end
  return x
end