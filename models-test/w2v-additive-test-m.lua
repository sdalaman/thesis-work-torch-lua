...
function test_model(model,final,epoch,inputs_pri,inputs_sec,test_size,output_file)
  ...
  vs1 = all_rootsPri:size(2)
  vs2 = all_rootsSec:size(2)
  if vs1 > vs2 then
    all_rootsPri = all_rootsPri[{{},{1,vs2}}]
  end
  if vs2 > vs1 then
    all_rootsSec = all_rootsSec[{{},{1,vs1}}]
  end
...
cmd:option("-dtype1", config.dtype1)
cmd:option("-dtype2", config.dtype2)
...
summary_file:write("\n")
summary_file:write(config.dsize..","..config.dtype1..","..config.dtype2..","..config.lang1..","..config.lang2..","..totscore.."/"..totalLines..","..totscore/totalLines)
summary_file:write("\n")
